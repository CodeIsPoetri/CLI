...
module.exports = class Project {
    constructor (directory, options = {}, repository) {
        const { slug, name, description, functions = [] } = options;
        Object.assign(this,
            { description, directory, functions, name, slug });
    }
    ...
    static async write (project) {
        const { directory, name, slug, description, functions } =
            project;

        // Writes project file as {directory}/project.yml
        const data = YAML
            .stringify({ name, slug, description, functions });
